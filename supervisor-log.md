...
**A process fact that cost this leg about twenty minutes and will cost the next one the same.** The reviewer **finished in 77 seconds** and its completion notification reached me roughly **twenty minutes later**. I spent that gap polling `inbox/` for a drop, which cannot distinguish "no findings" from "still running" — the absence of a drop is exactly what a clean review looks like. So the poll I reached for is structurally incapable of answering the question, and the only real signal is the notification. I waited rather than writing `skipped`, which was right, but for the wrong reason: I was following the rule, not reading a signal. **The rule is load-bearing precisely because the cheap probe is useless here** — worth knowing before a leg invents its own liveness check, which `topology/006` already did once and got wrong in the other direction.
